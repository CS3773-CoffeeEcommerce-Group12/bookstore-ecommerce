...
export default function AdminFulfillments() {
  ...
  // Fetch cancelled fulfillments
  const { data: cancelledFulfillments = [], isLoading: loadingCancelled } = useQuery({
    queryKey: ['fulfillments', 'cancelled'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('order_fulfillments')
        .select('*')
  ...
  // Fetch delivered fulfillments
  const { data: deliveredFulfillments = [], isLoading: loadingDelivered } = useQuery({
    queryKey: ['fulfillments', 'delivered'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('order_fulfillments')
        .select('*')
    ...
    refetchInterval: 60000, // Less frequent for delivered items
  });

  // Fetch fulfillment statistics
  const { data: stats, isLoading: loadingStats } = useQuery({
    queryKey: ['fulfillments', 'stats'],
    queryFn: () => fulfillmentService.getFulfillmentStats(),
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {isLoading ? (
          ...
          {/* Nested tabs for different ongoing statuses */}
          <Tabs defaultValue="pending" className="space-y-4">
            <TabsList className="bg-muted border border-border p-1">
              <TabsTrigger value="pending" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
                ...
                Pending ({pendingFulfillments.length})
              </TabsTrigger>
              <TabsTrigger value="processing" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
                <Package className="h-4 w-4 mr-2" />
                Processing ({processingFulfillments.length})
              </TabsTrigger>
              <TabsTrigger value="shipped" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
                <Truck className="h-4 w-4 mr-2" />
                Shipped ({shippedFulfillments.length})
              </TabsTrigger>
            </TabsList>
                ...
                <Card>
                  <CardContent className="py-12">
                    <div className="text-center">
                      ...
                      <p className="text-muted-foreground">No pending fulfillments at the moment.</p>
                    </div>
                  </CardContent>
                </Card>
              )}
                ...
                <Card>
                  <CardContent className="py-12">
                    <div className="text-center">
                      ...
                      <p className="text-muted-foreground">Orders currently being processed will appear here.</p>
                    </div>
                  </CardContent>
                </Card>
              )}
            ...
            <TabsContent value="shipped">
              {loadingShipped ? (
                ...
                <Card>
                  <CardContent className="py-12">
                    <div className="text-center">
                      <Truck className="h-16 w-16 mx-auto text-purple-500 mb-4" />
        ...
        <TabsContent value="completed">
          ...
          ) : deliveredFulfillments.length > 0 ? (
            <div className="space-y-4">
              {deliveredFulfillments.map((fulfillment) => renderFulfillmentCard(fulfillment as FulfillmentWithDetails))}
            </div>
          ) : (
            <Card>
              <CardContent className="py-12">
        ...
        <TabsContent value="cancelled">
          {loadingCancelled ? (
            ...
            <div className="space-y-4">
              {cancelledFulfillments.map((fulfillment) => renderFulfillmentCard(fulfillment as FulfillmentWithDetails))}
            </div>
          ) : (
            <Card>
              <CardContent className="py-12">
